...
```sh
npx wrangler kv namespace create TL
```

### 3. Configure `wrangler.toml`

Copy `wrangler.toml.example` to `wrangler.toml` and fill in your R2 bucket name and the KV namespace ID from step 2.
...
npx wrangler secret put PUBLIC_URL        # URL where posts are visible, e.g. "https://example.com/tl/"
...
```sh
curl "https://api.telegram.org/bot<YOUR_TOKEN>/setWebhook?url=https://tl-worker.<your-subdomain>.workers.dev/webhook"
```

## Post format

Each post is a markdown file committed to `src/content/timeline/{slug}.md`:
...
<img src="https://files.example.com/tl/1736942400.jpg" width="400" height="300" alt="">
```
...
The content path (`src/content/timeline/`) and R2 key prefix (`tl/`) are set in `index.ts`. Change them to match your site structure.
